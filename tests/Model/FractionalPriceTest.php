...
class FractionalPriceTest extends TestCase
{
    public function testCurrencyValidation(): void
    {
        ...
        $violations = $validator->validate(new FractionalPrice(Decimal::fromString('10'), 'EUR'));

        $this->assertCount(0, $violations->findByCodes(Currency::NO_SUCH_CURRENCY_ERROR));

        $violations = $validator->validate(new FractionalPrice(Decimal::fromString('10'), 'foo'));
