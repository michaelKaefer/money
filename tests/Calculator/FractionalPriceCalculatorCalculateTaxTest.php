...
class FractionalPriceCalculatorCalculateTaxTest extends TestCase
{
    ...
    {
        $taxRate = $this->calculator->calculateTax($a, $b);

        $this->assertInstanceOf(Decimal::class, $taxRate);
        $this->assertTrue($taxRate->equals($expected), sprintf('In Unit\Money\Calculator\FractionalPriceCalculator on tax calculation the amount %s is equal to the expected amount %s', (string) $taxRate, (string) $expected));
    }
